...
const HospitalLogin = () => {
  ...
  // Get the hospital data from the Redux store
  const hospital = useSelector((state) => state.hospital);

  // Redirect to hospital page if logged in
  ...
  const handleLogin = async (e) => {
    ...
    if (email && password) {
      try {
        setLoading(true);
        await Hospitallogin(dispatch, { email, password });
        setLoading(false);
      } catch (error) {
        setLoading(false);
      }
        ...
        <div className="p-10 w-[500px]">
          ...
          <form className="space-y-5">
            ...
            <div>
              ...
              <Link to="/hospitalregister" className="hover:text-gray-600 font-semibold w-6 h-6 ml-5 text-lg">
                Create an Account
              </Link>
            </div>
          </form>
